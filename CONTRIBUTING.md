...
- Tail the log by running `make logs`
- Run `dash` in debug mode with `make debug` in another terminal window / pane
...
CI runs this check, but for PRs from forks it may require maintainer approval before it starts. Running `make lint` locally saves a round-trip.
...
- Run the docs site by running `make docs`
