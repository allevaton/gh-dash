...
These settings control behavior specific to the PR detail/preview view.

#### Summary Fold Height (`foldBodyHeight`)

| Type    | Minimum | Default |
| :------ | :-----: | :-----: |
| Integer |    0    |    8    |

Specifies the maximum number of lines of the PR description (the "Summary" panel) to display
before folding the rest behind a "Press e to read more..." prompt. Press <kbd>e</kbd> to
toggle the full description.

Set to `0` to disable folding entirely and always render the full description.

By default, descriptions taller than 8 lines are folded.
